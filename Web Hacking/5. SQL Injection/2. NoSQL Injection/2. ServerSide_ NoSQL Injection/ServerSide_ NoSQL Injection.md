...
위의 코드는 **입력값과 타입을 출력**하는데 ``req.query``의 타입이 **문자열로 지정되어 있지 않기** 때문에 **문자열 외의 타입이 입력될 수** 있다.  

아래의 코드는 위의 코드 실행 결과로, 각 **데이터의 type 을 출력**하는 코드다.
...
위의 코드는 user 컬렉션에서 이용자가 입력한 **uid, upw** 에 해당하는 **데이터를 찾고 출력** 하는 예제 코드다.

입력값에 대해 **타입을 검증하지 않기** 때문에 오브젝트 타입의 값을 입력할 수 있다.
...
**$ne 연산자** 를 사용해 uid와 upw가 **"a"가 아닌 데이터를 조회** 하는 공격 쿼리와 실행 결과다.

## NoSQL Injection 실습(개정 이전 실습)

<img src="5.jpg">  

다음은 모듈을 구성하는 코드다. 위에서 봤던 코드와 비슷하다.
...
Blind NoSQL Injection 는 **참/거짓 결과**를 통해 **데이터베이스 정보**를 알아낼 수 있다.
...
|**$text**|지정된 **텍스트를 검색**한다.|
|**$where**|**JavaScript 표현식을 만족하는 문서**와 일치한다.|
...
위의 그림을 보면 **{"$regex":"^a"}** 를 해서 result가 **uid**가 나온 것을 보고, **첫 번재 글자는 a** 라는 것을 알 수 있다.

이와 다르게 **{"$regex":"^b"}** 를 적으면 result 에서 **undefined** 를 출력하는 것을 볼 수 있다.

이렇게 하나씩 넣어서 알 수 있다.

<img src="16.jpg">  

결국 비밀번호가 **apple** 인 것을 알 수 있다.

## Blind NoSQLi 실습 풀이

<img src="17.jpg">  

풀이를 보던 중 **{"$regex":".{5}"}** 등을 통해 길이를 알 수 있다는 것을 알았다.
